    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      ...
      <Card style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'var(--space-7)', flexWrap: 'wrap',
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontFamily: SF, fontSize: '5.5rem', lineHeight: 1, color: 'var(--accent-sheet)' }}>{syllable}</div>
      ...
      <Card>
        ...
        <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
          {consonants.map((cc, idx) => (
            <Chip key={idx} active={idx === ci} accent={cc.accent} onClick={() => setCi(idx)} font={SF}>{cc.char}</Chip>
          ))}
        </div>
      </Card>
