...
function ProgressScreen({ lang, theme, cloudStatus, onCloud, onGo, user, onSignIn, onSignOut }) {
      ...
      <Card>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 'var(--space-4)', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)', minWidth: 0 }}>
            <span style={{ width: 44, height: 44, borderRadius: '50%', display: 'grid', placeItems: 'center', flexShrink: 0, fontWeight: 700,
                           color: user ? '#fff' : 'var(--text-secondary)', background: user ? 'var(--gradient-display)' : 'var(--bg-secondary)',
                           border: user ? 'none' : '1px solid var(--border-color)' }}>
              {user ? (user.email || '?').slice(0, 1).toUpperCase() : <Icon name="user" size={20} />}
            </span>
            <div style={{ minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700 }}><Icon name="cloud" size={18} /> {user ? 'Synced to your account' : 'Saved on this device'}</div>
              <div style={{ fontSize: 'var(--fs-small)', color: 'var(--text-secondary)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {user ? (cloudStatus === 'error' ? `${user.email} · last sync failed — try Pull latest` : user.email)
          ...
          <span style={{ fontSize: 'var(--fs-hint)', fontWeight: 700, padding: '0.3rem 0.8rem', borderRadius: 'var(--radius-pill)',
                         color: '#fff', background: user ? (cloudStatus === 'error' ? 'var(--error)' : 'var(--success)') : 'var(--text-muted)' }}>
            {user ? (cloudStatus === 'error' ? 'ERROR' : 'SYNCED') : 'DEVICE ONLY'}
          </span>
        </div>
        <div style={{ marginTop: 'var(--space-4)', display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
          {user ? (
            <React.Fragment>
              <Button accent="quiz" icon={<Icon name="download" size={16} />} onClick={() => onCloud && onCloud('pull', lang.id)}>Pull latest</Button>
              <Button variant="secondary" icon={<Icon name="logout" size={16} />} onClick={onSignOut}>Sign out</Button>
            </React.Fragment>
          ) : (
            <Button accent="practice" icon={<Icon name="user" size={16} />} onClick={onSignIn}>Sign in</Button>
          )}
        </div>
      </Card>
    </div>
