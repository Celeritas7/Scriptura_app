...
- `design-system/tokens/` — colors, typography, spacing, effects (CSS custom properties).
- `design-system/components/` — reusable React primitives (Button, NavButton, Toggle, RangeSlider, SegmentedControl, Tab, Card, Pill, StatChip). Each has a `.prompt.md` with usage.
- The app itself lives at the repo root (`index.html`, `screens/`, `components/`, `lib/`, `data/`) — copy screens from there.
