...
All faces load from **Google Fonts** (`design-system/tokens/fonts.css`) — these are the *real* fonts
...
- `design-system/tokens/` — `fonts.css`, `colors.css`, `typography.css`, `spacing.css`, `effects.css`.
- `readme.md` — this file.
...
`design-system/components/` — reusable React primitives (namespace `ScripturaDesignSystem_72b484`):
...
Repo root (`index.html` + `screens/`, `components/`, `lib/`, `data/`) — the app: full interactive recreation, redesigned as a **guided,
